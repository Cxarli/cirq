/* globals Circuit, CustomGate, LOOP_OBJ, CLONE, Gate, Wire, Port */
...
function loadCustomGateJSON(json) {
    ...
    // Backup and reset wires so as to not interfere
    let _wires_bak = window._wires;
    window._wires = [];
    ...
    */
    ...
    let gates = LOOP_OBJ(json.circuit.gates).map(
        (uuid, data) => {
            ...
            // Copy port names
            LOOP_OBJ(data.ports).forEach((portuuid, port) => {
                console.log(portuuid);

                // function Port(gate, id, type, name, width)

                gate.ports[portuuid] = new Port(gate, portuuid);

                gate.ports[portuuid].name = port.name;
                gate.ports[portuuid].nameset = true;
            });
    ...
    let wires = json.circuit.wires.map(
        (data) => {
            ...
            let wire = new Wire(
                gates[uuida].getPortByName(porta),
                gates[uuidb].getPortByName(portb)
            );
